...
classify_pbmc_cells <- function(seurat_data, path_10x_data, sample_names = FALSE,
                                marker_file_path, lower_gene_filter_num = 200,
                                pbmc_classifier = FALSE,
                                cds_gene_type = "ENSEMBL", downsampled = TRUE,
                                save_object = TRUE,
    ...
    if (isTRUE(downsampled)) {
        ...
        for (path in path_10x_data) {
            ...
            SAMPLE <- load_cellranger_data(sample_path,
                umi_cutoff = lower_gene_filter_num
            )
